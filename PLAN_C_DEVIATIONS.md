...
The test compiled clean and produced the correct sorted output to stdout, but the process was killed by SIGSEGV on exit. All four PR #109 CI lanes failed on this single test; 418 sibling tests passed.

Bisecting the source produced a 12-line reproducer that did NOT involve `list_sort` or `string_compare` — just a `match` with a `Cons(h, _) => { perform IO.println(h); 0 }` arm body. The bug also reproduced on `origin/main` commit `f96e77c` (the last commit before the Stage MOS branch diverged), confirming pre-existing latency. The first round of PR #109 review-fix commits silently worked around it with the same split-fn pattern that spec example E15 uses for `dump_each` / `dump_each_step`. PR #109 review #2 rejected the silent treatment as an LLM-first-thesis-eroding pattern (idiomatic-looking code fails at runtime, no diagnostic, the workaround propagates without a signal) and required either filing the deviation + queuing a fix plan + documenting the pattern, OR fixing the underlying bug. The user picked the underlying fix.

**Root cause.** The synth-cont generated by codegen for the parent helper's compound-match-with-arm-perform shape (one synth-cont per `PerformThenTail` arm, allocated at lines 10269–10291 of `compiler/src/codegen.rs`) reads its post-arm-k pair from `args_ptr+POST_ARM_K_CLOSURE_OFF` (= 8) and `args_ptr+POST_ARM_K_FN_OFF` (= 16) at synth-cont entry (lines 14409–14467). The Slice A convention (Plan B Task 55 Phase 4e) says continuations dispatched from arm bodies follow the 3-slot shape `[arg, post_arm_k_closure, post_arm_k_fn]` — every k(arg) caller writes those slots.

User-defined arm fns honour the convention via `lower_k_pair_call` (lines 21804+): they allocate `sigil_next_step_call(k_closure, k_fn, 3)` and store `[arg, post_arm_k_closure_loaded_from_arm_closure_record, post_arm_k_fn_loaded_from_arm_closure_record]`. Builtin runtime arms (`sigil_io_println_arm`, `sigil_io_print_arm`, `sigil_io_read_line_arm` in `runtime/src/handlers.rs`) silently violated the convention: they allocated only `sigil_next_step_call(k_closure, k_fn, 1)` and stored only the `k(value)` argument, leaving slots 1 and 2 uninitialised in arena memory.

When the synth-cont read `args_ptr+8` / `args_ptr+16` after a builtin IO arm's k(value) dispatch, it read garbage (whatever was in the arena beyond the 1-slot allocation). For the failing literal-tail synth-cont shape, the synth-cont then dispatched `sigil_next_step_call(garbage_post_arm_k_closure, garbage_post_arm_k_fn, [tail], 1)` — the trampoline's next dispatch jumped to a garbage function pointer → SIGSEGV.

The recursive Cps→Cps tail shape (`print_list(t)`-style) happened to survive because the trampoline's DONE-path `outer_post_arm_k_try_pop` rarely consumed the pushed garbage entries during the ascent — the recursion bottomed out at a `ConstantDone` arm whose `done_or_dispatch_return_arm_ref` returned `Done(0)` BEFORE the chain unwind drove the garbage entries through `sigil_next_step_call`. That asymmetry made the bug visible only on the literal-tail variant.
...
1. Spec example E15 ALREADY silently works around the same bug with the split-fn pattern. Two known reproducers (E15 + this Stage MOS test) qualify the bug as recurring, not one-off.
2. The footgun is exactly the LLM-first-thesis-eroding shape — idiomatic-looking code crashing at runtime with no diagnostic.
3. The fix is small and localised (3 builtin arms + 2 helpers + 2 unit tests + 1 e2e regression), fits inside the same PR cycle, and eliminates the workaround propagation risk.

**Failure mode (pre-fix).** SIGSEGV with empty stderr after the perform's output reaches stdout. No diagnostic from the typechecker or the runtime; the crash signature is opaque to anyone who hasn't seen the symptom. Reproducer:

```sigil
fn first(b: Bool) -> Int ![IO] {
  match b {
    true => { perform IO.println("hi"); 0 },
    false => 0,
  }
}
fn main() -> Int ![IO] { first(true) }
```
