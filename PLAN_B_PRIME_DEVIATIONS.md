...
**(a) update — CLOSED 2026-05-03 by Plan D Task 111 (a + b + c + d).** The "packed multi-return" framing the original carryover proposed proved structurally insufficient (PR #50's three implementation attempts demonstrated the cross-fn discharge propagation requirement requires shared state, not a per-call mechanism — see `PLAN_D_DEVIATIONS.md` `[DEVIATION Task 111]` for the analysis). The shipped closure is option-(C) instead: thread `*mut TerminalResult` through every fn ABI as a trailing positional arg. Sync ABI gains `terminal_out` as the trailing param (PR #90 / Task 111b); Cps + synth fn ABI extends `cps_signature` to a 4-arg shape `(closure_ptr, args_ptr, args_len, terminal_out)` (PR #91 / Task 111c); 111d (PR #92) removes the TLS path entirely + adds snapshot/restore at every handle entry/exit so nested-handle inner-discharge state cannot leak to outer. The "architecturally fragile under future nested-handle shapes" concern is now both fixed (the snapshot/restore discipline is correct-by-construction) and tested (`task_111d_nested_handle_inner_discharge_does_not_leak_to_outer` pins the invariant). Plan D Task 111 closes carryover #1 in full.

**(b) update — STILL OPEN.** Sync shim emission gating remains a Plan-C-completion / future-PR item; tracked as a named issue at https://github.com/boldfield/sigil/issues/48 per Plan D Stage 10.5.6's carryover-tracking discipline.
