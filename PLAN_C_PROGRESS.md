...
- Task 72 — Write `std/state.sigil` — `State[S]` effect + `run_state: (S, () -> A ![State[S] | e]) -> (A, S) !e`.
  - status: **part 1 done-pending-ci** ([HEAD]). Ships v1 surface: `effect State resumes: many { get: () -> Int, set: (Int) -> Int }` (concrete-Int, non-generic), `get_state()` / `set_state(s)` user wrappers, `run_state(initial: Int, body: () -> Int ![State]) -> Int ![]` discharger that threads state via the state-bearing-lambda pattern from examples/state.sigil (Plan B' Stage 6.8). Four v1 constraints force the simplification per `[DEVIATION Task 72]`: parser rejects type-parameterized effect references in rows (same as Task 71); no tuple type / Pair stdlib (so `run_state` returns just A, not (A,S)); no per-op generic params on user-declared effects; no row-poly Fn params yet. 3 typecheck unit tests + 2 e2e tests cover the canonical set+get composition + initial-value-pass-through. v2 generalisation lands when parser + tuple + per-op generics + row-poly Fn ship.
