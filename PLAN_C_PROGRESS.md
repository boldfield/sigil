...
- Task 65 — Write `std/array.sigil` — immutable `Array[A]`, `length`, `get`, `set` (returns new), `from_list`, `to_list`. Requires runtime support for array allocation; extend `runtime/`.
  - status: **part 1 done-pending-ci** ([HEAD]) — runtime foundation only. `runtime/src/array.rs` ships `sigil_array_alloc` / `_empty` / `_length` / `_get` / `_set` with 7 unit tests; TAG_ARRAY=0x04 in header-constants; 2 new counters (ArrayAllocCount, ArrayAllocBytes). Layout uses `count=0, bitmap=1` to sidestep the 6-bit count cap so arrays beyond 63 elements (Sudoku's 81-element board, etc.) work. **Part 2 PENDING** — typecheck builtin Array type registration + generic schemes for the 5 ops, codegen FFI declarations + dispatch, `std/array.sigil` (likely documentation-only), tests. See `[DEVIATION Task 65]` in `PLAN_C_DEVIATIONS.md` for the split rationale.
