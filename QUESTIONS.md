...
decision log. This file is preserved across all plans (A1, A2, A3, B, B', C, D)
so the rationale trail is single-threaded.
...
`[PLAN-B]`, `[PLAN-B-PRIME]`, `[PLAN-C]`, `[PLAN-D]`. Untagged entries are
legacy (pre-A2) and implicitly belong to `[PLAN-A1]`.
