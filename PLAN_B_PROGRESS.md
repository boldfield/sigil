...
- Task 50 — Color inference: per-monomorph, SCC-aware, `--dump-color`
  - status: done-pending-ci
  - commits: [HEAD]
  - notes: New `compiler/src/color.rs` (~610 lines incl. tests) replaces the Plan A1 always-Native stub. Per-monomorph local analysis classifies each top-level fn: open effect row (`![IO | e]`) → CPS; row contains a non-`IO` effect → CPS; body (and any nested lambda body) performs a non-`IO` effect → CPS; else native. Reason strings are stable and cover all four categories — the local-CPS reason is the first matching rule. Call-graph propagation: every `Expr::Call { callee: Ident(name), .. }` and every bare-`Ident` reference to a known top-level fn becomes an outgoing edge (the conservative-but-sound choice for Plan B v1; first-class fn values aren't statically resolvable here without closure conversion, but lambdas inside fn bodies get walked for performs/effects via local analysis already). Tarjan SCC over the edges in single-pass, sorted-set iteration — emits SCCs in reverse-topological order so propagation can fold callee-SCC colors into caller-SCC colors as it goes. Within an SCC, if any member is locally CPS the whole SCC is CPS (reason: `cps: in SCC with cps member <name>` for non-intrinsic members); otherwise if any member calls into a CPS-colored downstream SCC, the SCC is CPS (reason: `cps: transitively calls <callee> which is cps`). Otherwise native. **`--dump-color` CLI flag** lands in `cli.rs` + `main.rs` + new `pipeline::dump_color` helper: runs lex→parse→resolve→typecheck→elaborate→monomorphize→color and prints `<name> native|cps <reason>` per fn (one line per monomorph in program order) to stdout, no codegen. `-o` is silently accepted but ignored under `--dump-color`; `--print-runtime-stats` conflicts and emits a usage error. 15 color::tests unit tests cover open-row CPS, non-IO-row CPS, perform-non-IO CPS (via row check), perform-non-IO-with-IO-row CPS (via body walk), caller-callee both native, mutual recursion native, mutual recursion with one CPS member tainting whole SCC, transitive CPS taint, unrelated cycle non-pessimization, lambda body perform tainting parent, dump_color stable per program order, and `--dump-color` ignores `-o` / conflicts with `--print-runtime-stats` (4 cli::tests). 2 e2e tests (`dump_color_hello_is_native_row_io`, `dump_color_multi_fn_pure_program`) exercise `sigil <input> --dump-color` end-to-end. Pod-verify green; full lib tests deferred to CI.
