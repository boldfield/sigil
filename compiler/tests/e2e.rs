...
#[test]
#[ignore = "Cross-handler state-passing partial. Slice C multi-shot + return arm now works via the CPS path (synth-cont applies return arm before binding; arm chain Final emits Discharged with own effect_id; see slice_c_choose_multi_shot_with_return_arm_collects_both_branches). The State handler's `fn(s) => k(s)(s)` lambdas are wrapped via wrap_continuation_with_outer_post_arm_k at perform time so the Amb chain entries survive cross-handler captures, and lower_k_pair_call propagates foreign discharges (Amb's eid leaking into State's lambda body) so the outer apply on `k(s)` doesn't dereference the List[Bool] discharge value as a closure. Remaining gap: the propagated discharge value reaches main as Nil (length 0 reported instead of 2). Likely one of: (1) sync-shim's run_loop drive returning a stale arena pointer instead of the heap append_result; (2) terminal_out.value getting overwritten between arm_step_1's emit and the propagation read; (3) Sync caller (run_state_poly's body) reading the lambda's return as the NextStep ptr u64 rather than the post-bypass terminal_out.value. The simple_amb (no State performs in body) and plotkin_lite (State outer + Amb inner, body lacks State performs) variants both pass."]
