...
| `std.string` | Byte-indexed string ops: `string_concat`, `string_substring`, `string_byte_at`, `string_starts_with`, `string_ends_with`, `string_contains`, `string_index_of`, `string_trim`, `string_length`. Lexicographic comparison is `string_compare` from `std.ordering` (returns `Ordering`). Codepoint-indexed: `string_chars`, `string_char_at`, `string_from_chars`. Pure-Sigil helpers: `string_split`, `string_replace`. Decimal-integer parse: `string_to_int(s) -> Result[Int, ParseError]` is the canonical surface (`ParseError = \| Empty \| NonDecimal \| Overflow`); the underlying `string_to_int_validate` (Int code) / `string_to_int_parse` (raw `Int`, validation-required) builtins remain available for callers that want to skip the sum-typed wrapper. |
