...
- **Stdlib migration deferred to Plan C completion** — std/raise.sigil + std/state.sigil + std/result.sigil. Migration attempted in this session surfaced an additional **handler-discharge type-arg propagation gap**: when `handle X.op(args, k) => ...` discharges a generic effect-decl like `Raise[E]`, the body row's discharged `EffectInst::bare(name)` doesn't carry the args. body row passthrough (e.g., `body() -> A ![Raise[E] | e]` invoked inside the handle) fails E0042 because the row-discharged `Raise` (no args) doesn't match `Raise[E]` (args). Closure path: extend `discharged: Vec<String>` in `Tc::check_handle` to track args (likely via inferring from the handle body's typed effect set, or requiring the user to specify `handle X.op[Type] => ...`) — a follow-up task. The migration lands in Plan C completion alongside the handler-discharge gap fix.
