...
- Task 111 — TLS → out-pointer `TerminalResult` for `sigil_run_loop` terminal (Plan B' carryover #1, PR #39 §2).
  - status: done-pending-ci ([HEAD]) — `sigil_run_loop` now takes a second `*mut TerminalResult` parameter and writes `(value: u64, tag: u64)` to `*out` before returning. Runtime: 2 TLS cells (`LAST_TERMINAL_TAG`, `LAST_TERMINAL_VALUE`) and 4 FFI helpers (`sigil_last_terminal_tag` / `_value` / `sigil_reset_*`) deleted; `runtime/src/handlers.rs` carries no globals for terminal tracking; 13 in-file unit tests updated to stack-allocate `TerminalResult` and pass via `&mut`. Compiler: `run_loop_sig` updated to two-pointer params; deleted 4 FFI declarations + 4 `FuncId` fields on `PerFnRefsCtx` + 4 `FuncRef` fields on `PerFnRefs` and `Lowerer` + ~13 threading sites; added 2 `Option<Variable>` fields on `Lowerer` (`last_terminal_value_var`, `last_terminal_tag_var`) with `last_terminal_vars` (lazy declare-and-init), `reset_last_terminal_vars`, and `emit_run_loop_and_capture` helpers; updated 5 internal `run_loop_ref` call sites and 1 Sync-shim call site to use the new helper / explicit stack-slot pattern; updated 2 handle-entry reset emits and 5 handle-exit query emits to use `def_var` / `use_var`. **PR #50 first-attempt note:** initial implementation used a register-pair multi-return signature with `#[repr(C)] struct { u64, u64 }` Rust return; PR #50's first CI run failed 10 e2e tests because the codegen-side `inst_results[0]` / `[1]` slot ordering did not match the runtime's struct field order on x86_64 (catch test returned 49 vs expected 42; run_state returned the lambda heap pointer vs expected 11). Pivoted to out-pointer convention which makes struct field offsets (0 for value, 8 for tag) the canonical contract on both sides — no register-pair ABI ambiguity. pod-verify clean.
