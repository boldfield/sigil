...
- Task 112 — Wrapper-fn-frame composition fix (closes `[DEVIATION Task 72]` constraint #3).
  - status: **112a done (PR #83); 112b deferred** — see `[DEVIATION Task 112a]` + `[DEVIATION Task 112b]`. Original deferral reopened 2026-05-03 after Task 117 substrate confirmed disjoint. User authorized single-PR architectural slice; implementation surfaced that tail-perform Cps wrappers compose cleanly while chained-let-yield Cps wrappers need additional codegen mechanism. User then split into 112a (tail-perform shipped this PR) + 112b (chained-let-yield deferred to follow-up).
- Task 112a — Tail-perform Cps wrapper composition.
  - status: **done** — PR #83 (squash `49bb595`). Chained-let-yield classifier accepts `let _ = wrapper_call(args)` shapes when callee is tail-perform Cps. Helper-body and Middle-step emit thread chain k-pair through wrapper boundary via trailing-pair args-buffer convention. Risk 3 BODY_RETURN_ARM PUSH/POP discipline added. 4 new e2e tests; un-ignored `std_state_run_state_via_wrappers_pending_v2_wrapper_fn_frame_fix`. Pre-existing tests preserved (`task_78_5_g4_approach6_risk3_*` falls back to Sync ABI for chained-let-yield wrappers).
- Task 112b — Chained-let-yield Cps wrapper composition.
  - status: **todo** — see `[DEVIATION Task 112b]`. Wrapper-shape-conditional OUTER_POST_ARM_K_STACK push at CallCps emit (push only when callee is chained-let-yield Cps, not tail-perform). Architecturally similar to Task 112a's scope (~200-400 lines codegen + tests). Plan D Task 119 closeout MAY wait for 112b shipping if JSON parser-half (or other smoke-gate consumer) requires chained-let-yield wrapper helpers.
