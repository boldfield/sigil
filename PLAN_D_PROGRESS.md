...
- Task 111 — TLS → out-pointer `TerminalResult` for `sigil_run_loop` terminal (Plan B' carryover #1, PR #39 §2).
  - status: in-progress (PR #50, two pivots; see `[DEVIATION Task 111]`) — runtime side complete (`sigil_run_loop(initial, out: *mut TerminalResult)`; 2 TLS cells + 4 FFI helpers deleted). Compiler side: 4 FFI declarations + their threading deleted; 5 internal `run_loop_ref` call sites and 1 Sync-shim call site rewired. **CI status:** PR #50 attempt 1 (register-pair multi-return, `4dfdbc7`) failed 10 e2e tests with discharge-class shapes; attempt 2 (out-pointer ABI, `670f7a1`) failed the same 10 tests with the same shapes. Reviewer (boldfield, 2026-04-30) diagnosed the residual bug as Cranelift `Variable` plumbing across blocks (name-based SSA fails to thread `def_var` to `use_var` when the body's control flow has paths that don't reach the run_loop call site). Attempt 3 in progress: pivot from `Variable` to per-fn `StackSlot` so reads/writes are explicit memory operations. Status flips to `done-pending-ci` only after attempt 3's CI is green. Closes once smoke gate passes.
