...
## 2026-05-03 — [DEVIATION Task 112b] Chained-let-yield Cps wrapper composition — CLOSED

**Context.** Task 112's split into 112a (tail-perform, shipped via PR #83) + 112b (chained-let-yield, this entry). The original deferral entry (drafted alongside the PR #83 closeout) named the closure path as a "wrapper-shape-conditional `OUTER_POST_ARM_K_STACK` push" — that diagnosis was inherited from PR #83 commit `7b56eec`'s revert message and was incorrect. The architectural read at the start of Task 112b's PR (PR #85) demonstrated that the failure mode is lambda-chain breakage at the wrapper boundary, not OUTER_POST_ARM_K_STACK routing — closure path is **caller-k_pair forwarding through chain step closure records with a runtime gate at the Final-step emit**.

**Failure mode (pre-fix).** Discharge-with-lambda handler arm (`fn(s) => k(arg)(arg)` state-threading shape) called against a body that uses a chained-let-yield Cps wrapper helper. The wrapper's chain Final dispatched its tail value via `emit_dispatch_to_post_arm_k(tail)`, loading post_arm_k from the chain step's args_ptr trailing slots. Inside a wrapper-in-chain caller, the args_ptr post_arm_k pair is `(null, identity)` (set by the lifted lambda's `lower_k_pair_call`) — equivalent to caller_k_pair at top-level only. In the wrapper-in-chain case, dispatching `(null, identity)` terminates the run_loop with the wrapper's tail instead of routing back to the calling chain step's synth-cont. The lambda chain breaks; the calling chain step's binding never sees the wrapper's tail value; downstream computations operate on stale state.

Pre-fix, this case was masked: classifier rejected chained-let-yield Cps wrappers as let-RHS callees (Task 112a's `is_tail_perform_cps_user_fn`); body fell back to Sync ABI; `lower_call`'s Cps branch handled the wrapper via SAVE+CLEAR+RESTORE BODY_RETURN_ARM_STACK pattern. Sync ABI dispatch produces the wrong answer for state-threading-shape handlers because the nested run_loop returns DISCHARGED with the lambda but the Sync caller treats it as the wrapper's "return value", breaking the lambda chain at a different point.

**Closure path.** Three coordinated changes in `compiler/src/codegen.rs`:

1. **Classifier widen.** New helper `is_supported_cps_user_fn` accepts both tail-perform (Task 112a) and chained-let-yield Cps wrappers as let-RHS callees. Conservative inner restriction: a chained callee's own let-RHS must be direct performs (not nested wrapper-Calls); wrapper-of-chained shapes still fall back to Sync.

2. **Caller-k_pair plumbing.** Every chained-let-yield body's chain step closure record carries the helper's `caller_k_pair` as 2 extra trailing slots after captures + prior_bindings (`caller_k_closure` + `caller_k_fn`). Helper-body alloc loads `caller_k_pair` from the helper's `args_ptr` trailing slots (same offsets tail-perform wrappers read). Each Middle step copies `caller_k_pair` from this synth-cont's record into the next step's record so it propagates end-to-end.

3. **Final-step runtime gate.** At the chain Final's emit, branch on whether the loaded `post_arm_k_fn` (from this synth-cont's args_ptr trailing slots) equals `identity_fn_addr`:

   - `post_arm_k_fn == identity` (canonical tail-k from `lower_k_pair_call`): use `caller_k_pair` dispatch with 3-slot args buffer `[tail, null, identity]`. At top-level (`caller_k_fn == identity_k_fn` from PR #80's pack) the dispatch routes via identity → `Done(tail)`, matching prior behavior. Inside a wrapper-in-chain caller, `caller_k_pair` routes `tail` into the calling chain step's synth-cont — closes the composition gap.
   - `post_arm_k_fn != identity` (Slice B `let r = k(arg); pure_tail` arm body): use the original post_arm_k dispatch (1-slot args buffer `[tail]`). The post-arm-k synth fn fires with `r = tail` and computes `pure_tail`. Slice B continues to compose correctly with top-level handle bodies.
