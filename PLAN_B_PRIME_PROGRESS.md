...
- Task 106 — B.3 acceptance e2e tests (id_fn-as-value; apply higher-order fn; make_adder fn-returning-fn; compose generic).
  - status: phase-c-plus-partial-done-pending-ci
  - commits: [HEAD]
  - notes: 4 positive e2e tests landed across Phase C v1 + Phase C+ Part 1 — `fn_as_value_via_let_binding_returns_42`, `higher_order_fn_param_returns_42`, `generic_apply_with_id_fn_returns_42`, **`make_adder_returns_12` (Phase C+ Part 1: call-returning-fn callee uses `call_callee_tys` side-table for sig resolution)**. R2 finding 2 added 3 more e2e tests: `fn_as_value_with_multi_param_returns_7`, `fn_as_value_with_effect_row_returns_42`, `closure_env_load_callee_is_e0138_until_phase_c_plus`. **Deferred to Phase C+ Part 2**: `compose` (lambda's body has fn-typed captures → ClosureEnvLoad-callees inside synth lambda fn). The `p17_compose_source_rejects_until_typeexpr_fn_ships` and `closure_env_load_callee_is_e0138_until_phase_c_plus` tests stay as rejection assertions until Phase C+ Part 2.
