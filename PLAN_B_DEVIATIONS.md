...
2. **`examples/state.sigil`** — uses two sequential `handle` expressions instead of a higher-order `run_state`. The first handle wraps `read_counter()` (which performs `State.get()`) and uses an arm that resumes with a fixed seed value; the second handle wraps `write_counter()` (which performs `State.set(arg)`) and uses an arm that resumes with the set arg (echo). Each handle registers BOTH `get` and `set` arms per the Phase 4f latent op_id/arm_count constraint (the `examples/div_recover.sigil` precedent). The "counter" framing is preserved at the source level: the read helper produces an incremented value (`count + 1`) and the write helper produces the previously-set value (echoed through `k`). The dual-handle pattern exercises both ops without needing a higher-order `run_state` or arm-body lambdas.
...
**Implementing commit(s):** Foundation `[HEAD]` (this entry + Task 58 squash-hash flip + PROGRESS Task 59 entry transition `todo` → `in-progress`); `examples/catch.sigil` + `catch_example_recovers_with_42` e2e at `[HEAD+1]`; `examples/state.sigil` + `state_example_dual_handle_returns_6_then_99` e2e at `[HEAD+2]`; `examples/choose.sigil` + `choose_example_dual_resume_returns_3` e2e at `[HEAD+3]`; closeout (PROGRESS Task 59 entry filled + this implementing-commit line backfilled) at `[HEAD+4]`.
