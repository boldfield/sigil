...
**Structural fix STILL deferred** to the future "Slice C captures-bearing extension" PR: thread user op-args INTO post_arm_k_1's closure record at the arm fn's body emit site, so post_arm_k_1's lowerer's env can resolve user-op-arg references in `arg2_expr`. The work is ~150-200 LOC across allocation site + env-construction site for post_arm_k_1 + same for post_arm_k_2 + GC bitmap update + tests. Bundles naturally with the N-chain extension since both extend post_arm_k_*'s closure-threading machinery; either can land first or together.
