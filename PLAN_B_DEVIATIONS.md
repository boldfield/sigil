...
**Walker-side fix deferred** to a separate post-Task-58 chore: extend `arm_body_unsupported_construct` to validate `arg2_expr`'s free vars are ⊆ `{binding_name_1, k_name, globals}` (mirroring `arm_body_post_arm_k_tail_free_vars_ok`'s shape, but excluding `binding_name_2` which doesn't exist yet at the post_arm_k_1 lowering site). The fix is mechanical (~10 lines, mirroring the existing free-var walker) and parallel work to the Slice C N-chain extension; either can land first. Both tracked under the broader "Slice C captures-bearing extension" closure point.

**Implementing commit(s) — CI fix #1**: `[HEAD+5]` rewrites both `examples/choose_demo.sigil` and `examples/multishot_stress.sigil` to use `r1 + N` for `arg2_expr`; updates docstrings to explain the constraint; this deviation entry's "Latent gap surfaced" + "Mitigation" + "Walker-side fix deferred" sub-sections were added in the same commit.
