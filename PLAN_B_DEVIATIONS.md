...
1. **Real CPS transform in `compiler/src/cps.rs`** (replaces the 21-line pass-through stub from Plan A1). Transforms CPS-color monomorph bodies into continuation-passing form: every `perform Effect.op(args)`, every call to a CPS-color user fn, and every non-tail expression whose continuation is non-trivial gets lambda-lifted into an explicit continuation closure. Pure parts of the body emit `NextStep::Done(value)` directly. The transform produces a new IR shape (`CpsExpr` or extension to `Expr` with a CPS-form variant) that codegen consumes to emit the CPS calling convention. Native-color monomorphs are passed through unchanged. The pass runs after `monomorphize` and `infer_colors`, before `closure_convert`, so closure-conversion's free-var analysis sees the lambda-lifted continuation closures.
...
4. **Colorer's handler-discharge refinement.** Phase 4d's `find_non_io_perform_in_expr` skips `Expr::Handle` bodies as inert from the surrounding fn's color perspective, recursing only into arm bodies. Phase 4e narrows this: the surrounding fn stays Native iff its handle bodies' performs are ALL discharged AT THE SYNTACTIC LEVEL (i.e., the perform appears textually inside the handle body, not transitively through a fn call). Cross-function-call performs taint the caller as CPS — because the colorer cannot prove (without whole-program data-flow) that the callee's perform reaches the discharging arm in tail position. This is the reviewer's open Stage-6 ask. The refinement uses the existing call-site instantiation table (`CheckedProgram::call_site_instantiations`) to drive an inter-procedural reachability check: for each handle body, walk its calls; if any callee transitively performs an effect the handle discharges, the calling fn is CPS. The walker is SCC-aware (already implemented in Phase 4d) so mutual-recursion through handle bodies doesn't over-pessimise.
...
e) **Colorer refinement uses the existing `call_site_instantiations` table** (Task 50 / Plan B). The table records every Ident-resolution-to-top-level-fn at typecheck time. Reachability through handle bodies extends the call-graph edge construction in `infer_colors` step 3: for each handle expression in fn `f`'s body, walk the handle's body sub-tree, harvest every direct call to a CPS-color top-level fn whose performs are not all discharged by the handle's arms. Add an edge from `f` to those CPS callees. The SCC propagation does the rest. The alternative (a fresh inter-procedural pass) would require its own data structure and propagation; reusing the call-graph keeps the colorer's machinery uniform.
...
- *Real CPS transform* — replaces `cps.rs` stub. Adds `Expr::CpsTailCall`, `Expr::CpsDone`, `Expr::CpsContinue` variants. Native fns pass through unchanged. CPS fns get their body recursively rewritten. Pod-verify by adding `cps::tests` covering the recursion shapes; full e2e exercise lands later.
- *Codegen consumes color* — CPS-color user fns get the uniform CPS ABI. Native-color callers of CPS callees get the inlined wrapper. Native fns containing handle bodies whose performs reach (post-colorer-refinement) CPS callees are re-classified — this is the big diff.
- *Colorer refinement* — `find_non_io_perform_in_expr`'s `Expr::Handle` arm narrowed to walk only direct (syntactic) performs; cross-call performs handled via call-graph edges in `infer_colors`. Tests pin both the discharge-via-direct-perform case (stays Native) and the discharge-via-helper-call case (becomes CPS).
...
- *Discard-k arm fires but rest-of-helper-body still runs (i.e., Phase 4d MVP behavior persists)* — colorer refinement commit. The colorer didn't recolor the helper fn as CPS; verify `find_non_io_perform_in_expr`'s handle-body-discharge logic correctly identifies cross-function-call performs.
