...
**Implementing commit(s):** Foundation `1fadef4` (this entry + Task 57 squash-hash flip + PROGRESS Task 58 entry transition `todo` → `in-progress`); `examples/choose_demo.sigil` + `choose_demo_example_returns_40` e2e at `3219092`; `examples/multishot_stress.sigil` + `multishot_stress_example_returns_1530` e2e at `3f872c5`; closeout (PROGRESS Task 58 entry filled + this implementing-commit line backfilled) at `312338f`; CI fix #1 + arg2 mitigation at `d60152b`; walker-side diagnostic at `44fd945`; final review fixups at `9995c51`; squash-merged at `c1d338c` (PR #31).

**Closure point:** Closed at `c1d338c` (PR #31 squash-merge, 2026-04-28) for the Task 58 *correctness rigor* dimension (multi-shot semantics work end-to-end with `(Int) -> Int` ops, fresh-frame independence asserted across 5 handles in one program). The literal "10+ resumes within a single arm" stress shape closes at the future Slice C N-chain extension PR — out of scope for Task 58, which is a test-rigor task per the plan's Stage 6 sequence.

**Latent gap surfaced by Task 58 implementation.** The first PR-#31 CI run failed (commits `1fadef4..312338f`) with `internal error: entered unreachable code: codegen: unknown ident "arg"` panicking at `compiler/src/codegen.rs:7765`. Root cause: Slice C v1's `post_arm_k_1` synth fn captures only `(k_closure, k_fn)` into its TAG_CLOSURE record at offsets 16/24 (per the comment block at codegen.rs:5480-5485); user op-args from the parent arm fn are NOT threaded into post_arm_k_1's env at construction (codegen.rs:6441-6442 only inserts `binding_name_1`). So if `arg2_expr` references a user op-arg (`arg`), `lowerer.lower_expr(&chain.arg2_expr)` at codegen.rs:6485 panics inside the `Expr::Ident` arm at codegen.rs:7765. Existing Slice C e2e tests (`slice_c_choose_multi_shot_*`) all use `Choose.flip: () -> Bool` (zero user op-args), so the case wasn't surfaced until Task 58 introduced the first `(Int) -> Int` multi-shot op. The walker (`arm_body_unsupported_construct` at codegen.rs:898-998) checks purity and tail-expr free-vars but does NOT validate `arg2_expr`'s free vars; it accepts `arg2_expr = arg + N` and lets codegen panic — a clean walker-side rejection with a Phase-4e-pointing diagnostic would surface the gap as a typecheck-style error instead of an internal-compiler-error panic.

**Mitigation in Task 58 v1**: rewrite both example arm bodies to use `r1 + N` for `arg2_expr` instead of `arg + N`. `r1` (the let-1 binding) IS in post_arm_k_1's env per codegen.rs:6442, so `lowerer.lower_expr` resolves it successfully. The closed-form expected outputs are preserved (since `r1 = k(arg + 100) = arg + 100` for the LetBindThenTail identity helper, `r1 + 100 = arg + 200` reproduces the original "arg + 200" semantics).

**Walker-side fix landed at `44fd945`** (separately from CI fix #1 to keep that commit focused on unblocking CI). `arm_body_unsupported_construct` (compiler/src/codegen.rs:944-994) now validates `arg2_expr`'s free vars are ⊆ `{binding_name_1, k_name, globals}` by reusing `arm_body_post_arm_k_tail_free_vars_ok` with `extra_bindings = {}` (no `binding_name_2` at the post_arm_k_1 lowering site). The diagnostic wraps the inner free-var check with arg2-specific Slice C framing pointing at the closure point: "Slice C: arg2 of multi-let arm body... post_arm_k_1's closure record... captures only (k_closure, k_fn), plus its own arg `r1`... Reference `r1` (the let-1 binding) or globals only, or defer to the future Slice C captures-bearing extension PR". New e2e `slice_c_arg2_referencing_user_op_arg_is_rejected_at_codegen` pins both the rejection-status and the diagnostic framing — at the future captures-bearing extension PR, this test inverts (becomes a positive test or moves to a separate accept-shape test) alongside the diagnostic-string update.

**Structural fix is post-Plan-B work** (not "future Plan B PR" — the gap stays open through Plan B closure). Threading user op-args INTO post_arm_k_1's closure record at the arm fn's body emit site (so post_arm_k_1's lowerer's env can resolve user-op-arg references in `arg2_expr`) is ~150-200 LOC across allocation + env-construction sites for post_arm_k_1 + same for post_arm_k_2 + GC bitmap update + tests. Reasoning for Plan-B-out-of-scope:

- **Task 58 closes today** (this PR); its scope is test-rigor, not codegen-feature.
- **Tasks 59–61 don't touch this surface** — Task 59 ships three demo programs (`catch.sigil`, `state.sigil`, `choose.sigil`) all expressible within Slice C v1's 2-let arm cap; Task 60's perf floor uses helper-recursion to *avoid* the N-chain limitation (the `pick_int(low, high)` idiom from `[DEVIATION Task 55] Phase 4e captures+` line 1507 produces 32+ leaves with 5 levels of recursion using only the 2-let arm shape); Task 61 authors P18-P20 prompt entries which, per the Stage 9 P20 readiness analysis, are expressible within v1.
- **The Stage 6 review checkpoint** doesn't gate on this — the linearity check, handler stack walk, multi-shot correctness, IO refactor, arena correctness, and color decisions reviewed there are all closed at PR #27/#28/#29/#30.
- **Plan B's scope guardrails** ("do not implement Stage 7+ features") apply: the structural fix is feature-extension territory, not a Stage 6 closure item.

So the structural fix sits as **Plan-C-or-later territory**, surfacing only when a real user program needs `arg2_expr` to reference a user op-arg. The walker-side diagnostic landed at `44fd945` covers the user-experience gap (clean compile error pointing at the closure point, instead of an internal-compiler-error panic) so the gap is *visible* to users without being blocking. Bundles with the N-chain extension if/when both surface together.

**Implementing commit(s) — CI fix #1**: `d60152b` rewrites both `examples/choose_demo.sigil` and `examples/multishot_stress.sigil` to use `r1 + N` for `arg2_expr`; updates docstrings to explain the constraint; this deviation entry's "Latent gap surfaced" + "Mitigation" sub-sections were added in the same commit.

**Implementing commit(s) — walker-side diagnostic**: `44fd945` extends `arm_body_unsupported_construct` to reject `arg2_expr` referencing names unavailable in post_arm_k_1's env (clean compile diagnostic instead of `unreachable!` panic); new e2e `slice_c_arg2_referencing_user_op_arg_is_rejected_at_codegen` pins the rejection + diagnostic framing. Final review fixups in `9995c51` (docstring drift in choose_demo / multishot_stress, closure-point reframing in this entry, runtime-independence note added).

## 2026-04-28 — [DEVIATION Task 59] Three demo programs fit the v1 expressibility envelope; literal "run_state" higher-order helper and "pair generator" two-perform helper body shapes deferred

**Context:** Plan B Task 59 calls for three example programs:

- `examples/catch.sigil` (Raise handler, one-shot).
- `examples/state.sigil` (`State[Int]` counter via `run_state`).
- `examples/choose.sigil` (nondeterministic pair generator; multi-shot).

The first program matches the existing v1 surface directly: a one-shot `Raise` effect with a recovery handler is the canonical Phase 4d / 4e shape (see existing e2e tests `discard_k_handler_*`, `phase4e_*_use_k`, etc.). The second and third programs each step into territory that touches v1 expressibility limits documented elsewhere — neither limit blocks Task 59's *demonstration* purpose, but the literal plan wording for each conflicts with v1's MVP shape envelope.

**`run_state` as a higher-order helper** is the standard algebraic-effects idiom: a top-level fn takes (initial_state, computation) and exposes State.get / State.set to the computation, classically implemented by handler arms returning lambdas-of-state which are applied to the initial state at the top level (Koka's `state[s] :: forall a. (() -> a) -> s -> (a, s)` shape). Sigil v1 cannot express this for two compounding reasons:

1. **`TypeExpr::Fn` is deferred** (per `examples/higher_order.sigil` docstring at lines 15-23): "a lambda cannot be let-bound, declared as a user fn parameter, or returned from a top-level fn". The `comp: () -> Int ![State, IO]` parameter shape `run_state` would need is not part of v1's surface syntax; lambdas exist but only in IIFE position.

2. **Handler arms cannot contain nested lambdas.** Phase 4d MVP's arm-body walker explicitly rejects `Expr::Lambda` and `Expr::ClosureRecord` in arm bodies (codegen.rs:1246-1257) with the diagnostic "contains a nested lambda — lambdas in arm bodies require a closure-convert side-table extension distinct from Phase 4d MVP". The lambda-of-state arm body shape (`State.get(k) => fn(s) { k(s)(s) }`) directly trips this gate.

So a literal `run_state` cannot ship in v1. Both lifts (TypeExpr::Fn and arm-body lambdas) are post-Plan-B / Plan-C-or-later territory.

**Two-perform helper bodies** (the natural shape for a "pair generator" — a helper that performs `Choose.flip` twice and returns a value computed from both outcomes) are not part of v1's `CpsContinuationKind` envelope. The user-fn pre-pass at codegen.rs:4289-4333 supports exactly two helper-body shapes: `ConstantDone` (`Stmt::Perform; IntLit(N)`) and `LetBindThenTail` (`let x = perform; pure_tail_using_x`). The docstring at codegen.rs:10286-10290 names "Multi-yield bodies (`perform; perform; tail`) — chained synth-conts" as a future widening explicitly deferred from Phase 4e captures+. So a helper performing Choose twice would need a chained-synth-cont extension that wasn't shipped — same future-widening territory as the Slice C N-chain extension referenced in `[DEVIATION Task 58]`.

**Deviation:** Task 59 ships:

1. **`examples/catch.sigil`** — straightforward fit. `effect Raise { fail: () -> Int }` declared at the top level; helper performs `Raise.fail()` once in a `LetBindThenTail` body that combines the perform's value with a captured user param; main wraps the call in `handle ... with { Raise.fail(k) => ... }` and the arm discards `k` to recover with a fixed value. Demonstrates one-shot effect dispatch, captures-bearing helper synth-cont, and discard-`k` semantics inverted by Phase 4e (the recover-with-constant value flows directly back as the handle expression's value, not through the helper's tail). E2e test pins the recovered value.

2. **`examples/state.sigil`** — uses two sequential `handle` expressions instead of a higher-order `run_state`. The first handle wraps `read_value()` (which performs `State.get()`) and uses an arm that resumes with a fixed seed value; the second handle wraps `write_value()` (which performs `State.set(arg)`) and uses an arm that resumes with the set arg (echo). Helpers are named `_value` rather than `_counter` to avoid suggesting cross-call mutable state — there is no threaded counter in v1, each handle is independent. Each handle registers BOTH `get` and `set` arms per the Phase 4f latent op_id/arm_count constraint (the `examples/div_recover.sigil` precedent). The dual-handle pattern exercises both ops without needing a higher-order `run_state` or arm-body lambdas; the read helper produces an incremented value (`count + 1`) and the write helper produces the previously-set value (echoed through `k`) at the source level.

3. **`examples/choose.sigil`** — uses the canonical Slice C v1 multi-shot pattern (single-perform helper + 2-resume arm body), framed as a "binary outcome enumerator" rather than a literal two-flip pair generator. `effect Choose resumes: many { flip: () -> Bool }`; helper performs `Choose.flip()` once and returns one of two integer values based on the bool; main's handler invokes `k(true)` and `k(false)` from a `{ let r1 = k(true); let r2 = k(false); r1 + r2 }` arm body, summing both branches' outcomes. Demonstrates multi-shot k via the canonical Slice C surface, with closed-form expected output. The "pair" is the (true_outcome, false_outcome) tuple of resume-result values, summed at the arm level.

**The literal interpretations of plan wording vs. v1 ship:**

| Plan wording | v1 ship | Closure point |
|---|---|---|
| "`State[Int]` counter via `run_state`" | Inline dual-handle pattern; `run_state` as a higher-order helper deferred | Future TypeExpr::Fn lift + arm-body-lambda lift (Plan-C-or-later) |
| "nondeterministic pair generator" | Single-flip multi-shot Choose with sum-of-branches arm | Future chained-synth-cont extension (multi-perform helper bodies) + Slice C N-chain extension |

Both deviations are **expressibility-bounded** — the v1 ships demonstrate the *intent* of each plan item (one-shot Raise recovery, State get/set via handler resume, multi-shot Choose with k-twice arm) without crossing into deferred surface. Task 59's demonstration purpose is preserved.

**Rationale:** Three options were considered for each example:

1. **Land the missing widening as part of Task 59.** For `state.sigil`'s `run_state`: this requires `TypeExpr::Fn` parsing/typecheck/codegen + arm-body-lambda lift + closure_convert side-table extension — multi-PR scope, well outside Task 59's Stage 6 closeout role. For `choose.sigil`'s two-perform helper: this requires the chained-synth-cont extension naming `Future widening from this slice (captures-bearing slice): 2. Multi-yield bodies (perform; perform; tail) — chained synth-conts` (codegen.rs:10289). Both are codegen-feature work, not test-rigor work.

2. **Defer Task 59 until the widenings land.** Pushes Stage 6 closure to an unknown future date and leaves the three demo programs uncovered. Task 60's perf floor and Task 61's prompt-bank work both reference the demos as canonical baselines; without Task 59 landing, those tasks have nothing to point at.

3. **(Selected.)** Land Task 59 with v1-permitted shapes for each demo and document the literal-plan-wording delta as future work. Matches the established Plan B precedent — Phase 4d MVP shipped under literal-Phase-4 wording with a captures+ deviation; Phase 4e captures+ shipped with a Slice C N-chain deviation; Task 58 shipped with a sequential-handles deviation; Task 59 ships with a `run_state` + two-perform-helper deviation. Each demo's intent is demonstrated; each literal-shape gap is explicitly named as Plan-C-or-later territory.

The selected option is consistent with Plan B's "do not implement Stage 7+ features" hard rule (`run_state` as a higher-order helper depends on `TypeExpr::Fn`, which is Stage-7-adjacent) and with the "expressibility envelope per slice" pattern established at Phases 4d / 4e / 4f / 4g / Task 58.

**Closure point — three sub-points**:

1. **`state.sigil`'s `run_state`**: future TypeExpr::Fn lift (post-Plan-B; `examples/higher_order.sigil` docstring's "deferred to Plan A3" framing was inherited, but Plan A3 closed without this surface; remains post-Plan-B) + arm-body-lambda lift (post-Plan-B chore distinct from Slice D's surrounding-lambda-captures shipped at PR #27).

2. **`choose.sigil`'s two-perform helper body**: future chained-synth-cont extension. Lifts `is_simple_let_yield_then_pure_tail_body`'s "exactly one let stmt" cap (codegen.rs:10303-10305) by allowing a chain of `let` stmts each binding a perform's result, with each successive synth-cont reading the prior synth-cont's bindings from its closure record. ~150 LOC across the pre-pass classifier + `CpsContinuationKind::ChainedLetBindThenTail` variant + chained synth-cont definition pass. Bundles naturally with the Slice C N-chain extension named in `[DEVIATION Task 58]` since both share the chained-closure-record discipline.
...
**Implementing commit(s):** Foundation `[HEAD]` (this entry + Task 61 squash-hash flip + PROGRESS Stage 6 cleanup section opened); A.1 E0142 partial-handler exhaustiveness at `[HEAD+1]`; A.3 binding_ty resolution at `[HEAD+2]`; A.2 IO color filter lift at `[HEAD+3]`; Phase 4f scope_id + Phase 4g cleanup at `[HEAD+4]`; Task 57 codegen cleanup at `[HEAD+5]`; closeout (PROGRESS Stage 6 cleanup entry filled + this implementing-commit line backfilled) at `[HEAD+6]`.
