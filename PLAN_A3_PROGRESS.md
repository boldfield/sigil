...
- Task 38 — Extend typechecker: nominal sum types + record field access + pattern matching with Maranget exhaustiveness
  - status: done-pending-ci
  - commits: [HEAD]
  - notes: Split into 38.1 (symbol table + Ty::User + E0112 unknown type + E0113 duplicate type), 38.2 (constructor resolution for positional Call + RecordLit), 38.3 (pattern typing: Var promotion to nullary Ctor, Tuple, Ctor; E0117 pattern type mismatch), 38.4 (Maranget's exhaustiveness + E0120 witness + E0130 reserved). Per reviewer constraint on PR #12, the staged E0111 gate held every well-formed ctor resolution from reaching codegen until Task 41 shipped. Flip commit (post-41.2) removes E0111 from the three resolution sites; renamed tests accordingly.
...
- Task 41 — Extend codegen: allocation, discriminant read, field load, record construction
  - status: done-pending-ci
  - commits: [HEAD]
  - notes: Split into 41.1 (constructor allocation) and 41.2 (match decision-tree lowerer). Flip commit removes the staged E0111 gate at all three ctor resolution sites (Unit / Positional / Record) plus the catalog entry; every well-formed ctor use now typechecks cleanly and flows into codegen. Renamed the three `_fires_e0111_and_resolves_type` tests to `_typechecks_cleanly` variants. 220 compiler lib tests pass (same count as post-41.2 — tests renamed/reworked, no additions).
